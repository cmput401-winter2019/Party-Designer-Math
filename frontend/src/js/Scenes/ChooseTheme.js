...
export class ChooseTheme extends Phaser.Scene {
    ...
    constructor() { super(CST.SCENES.CHOOSE_THEME); }
    preload(){
      // Load all themes
    ...
    setPage(){
        for(var i=0; i<this.themes.length; i++){
            ...
            let theme = new ThemeButton({scene:this, key:this.themes[i], x:newXPos*(1+(i%3)), y:newYPos, event:"pressed", iter: this.iter, player: this.player});
            theme.visible = false;
